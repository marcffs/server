...
while True:
    ...
    if request:
        ...
        #caso seja uma requisao PUT
        elif headers[0].split()[0] == "PUT" :
            #extraindo o conteudo da nova pagina a ser criada
            content = client_connection.recv(1024).decode()
            print(content)
            #atribuindo o caminho para o novo caminho para uma variavel
            newfile = open("htdocs" + filename, "w")
            #jogando o conteudo da nova pagina para dentro do novo arquivo
            newfile.write(content)
            #salvando o arquivo
            newfile.close();
            #envia a resposta com o caminho
            response = "HTTP/1.1 201 Created\nContent-Location: htdocs" + filename + "\n"
        else: 
            response = "HTTP/1.1 404 Not Found\n"
    ...
    else: print("HTTP/1.1 400 BAD REQUEST\n")
server_socket.close()